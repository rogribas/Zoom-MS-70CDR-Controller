...
void setup()
{
  ...
  updateLedsNumber(SendSysexGetProgram() + 1);
  // Update pedals
...
void loop()
{
  ...
  if (millis() - time > debounce) {
    if (readingUP == HIGH) {
      ...
      byte newPreset = (currentPreset + 1) % 50;
      SendChangeProgram(newPreset);
      updateLedsNumber(newPreset+1);
      previousUP = HIGH;
      ...
      byte newPreset = (currentPreset + 50 - 1) % 50;
      SendChangeProgram(newPreset);
      updateLedsNumber(newPreset+1);
      previousDOWN = HIGH;
